document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (megaGrid) {
    ...
    megaGrid.innerHTML = `
      ...
      ${groups.map((world) => `<div><h3>${esc(world)}</h3>${catalog.categories.filter(c => c.world === world).slice(0, 7).map(c => `<a href="${categoryHref(c)}">${esc(c.title)}</a>`).join('')}</div>`).join('')}
    `;
  }
  ...
  if (rail) {
    rail.innerHTML = catalog.categories.slice(0, 8).map((category) => `
      <a href="${categoryHref(category)}" class="codimas-category-pill">
        <span class="codimas-category-pill-media" style="background-image:url('${esc(getImage(category))}')"></span>
  ...
  if (categoriesGrid) {
    categoriesGrid.innerHTML = catalog.categories.map((category) => `
      <a href="${categoryHref(category)}" class="codimas-feature-category">
        <img src="${esc(getImage(category))}" alt="${esc(category.title)}" loading="lazy">
        <div class="codimas-feature-category-copy"><h3>${esc(category.title)}</h3><span>Explorar categoría →</span></div>
      </a>`).join('');
  }
  ...
  if (featuredGrid) {
    ...
    featuredGrid.innerHTML = featured.map((product) => `
      <article class="codimas-product-card">
        <div class="codimas-product-image"><img src="${esc(product.image_url || getImage(product.slug))}" alt="${esc(product.name)}" loading="lazy"></div>
        <div class="codimas-product-meta"><div class="category">${esc(product.category)}</div><h3>${esc(product.name)}</h3><p>${esc(product.spec || '')}</p><div class="code">Código ref. ${esc(product.code || 'S/C')}</div></div>
        <button class="quote-add-btn codimas-btn codimas-btn-dark" data-name="${esc(product.name)}" data-code="${esc(product.code || '')}" data-category="${esc(product.category)}" data-spec="${esc(product.spec || '')}">Agregar a cotización</button>
      </article>`).join('');
  }
  ...
  if (servicesGrid) {
    servicesGrid.innerHTML = catalog.services.map((service, index) => `
      <article class="codimas-service-card">
        ${service.image_url ? `<img src="${esc(service.image_url)}" alt="${esc(service.title)}" style="width:100%;aspect-ratio:16/9;object-fit:cover;margin-bottom:18px">` : ''}
        <span class="num">${String(index + 1).padStart(2, '0')}</span><h3>${esc(service.title)}</h3><p>${esc(service.text || '')}</p><a href="cotizacion.html?servicio=${encodeURIComponent(service.title)}">Solicitar cotización →</a>
      </article>`).join('');
  }

  const brandsGrid = document.getElementById('brands-grid');
  if (brandsGrid) brandsGrid.innerHTML = catalog.brands.map((brand) => `<div class="codimas-brand">${esc(typeof brand === 'string' ? brand : brand.name)}</div>`).join('');
  ...
  function wireSearch(formId, inputId, resultsId) {
    ...
    function render(query) {
      ...
      results.innerHTML = matches.length ? matches.map((category) => `<a href="${categoryHref(category)}"><strong style="display:block;font-size:13px;font-weight:600">${esc(category.title)}</strong><span style="display:block;margin-top:4px;color:#6c7076;font-size:12px">${esc(category.short || '')}</span></a>`).join('') : `<div style="padding:16px;font-size:13px;color:#6c7076">No encontramos coincidencias. Puedes describir el requerimiento en una solicitud de cotización.</div>`;
    }
