document.addEventListener('DOMContentLoaded', async () => {
  ...
  const imageFor = (item) => item?.image_url || defaultImages[item?.slug] || '../public/images/codimas-category.svg';
  ...
  root.innerHTML = `
    ...
    <main>
      ...
      <section class="codimas-page-hero"><div class="codimas-shell codimas-page-hero-grid"><div class="codimas-page-hero-copy"><p class="codimas-eyebrow">${category.world} · ${category.number || ''}</p><h1 class="codimas-heading" style="font-size:clamp(52px,6vw,94px);margin-top:18px">${category.title}</h1><p class="codimas-copy" style="margin-top:24px;max-width:680px">${category.description || ''}</p><div class="codimas-hero-actions"><a href="../cotizacion.html?categoria=${encodeURIComponent(category.title)}" class="codimas-btn codimas-btn-dark">Cotizar esta categoría</a><a href="#productos" class="codimas-btn codimas-btn-outline">Ver referencias</a></div></div><div class="codimas-page-hero-media" style="background-image:url('${heroImage}')"></div></div></section>
      <section class="codimas-section"><div class="codimas-shell"><div class="codimas-section-head"><div><p class="codimas-eyebrow">Subcategorías</p><h2 class="codimas-section-title">Encuentra la <strong>especificación correcta.</strong></h2></div><p class="codimas-copy" style="max-width:520px;font-size:15px">${category.quoteHint || ''}</p></div><div class="codimas-subcategory-grid">${(category.subcategories || []).map((item) => `<a class="codimas-subcategory-link" href="../cotizacion.html?categoria=${encodeURIComponent(category.title)}&producto=${encodeURIComponent(item)}"><span>${item}</span><span>→</span></a>`).join('')}</div></div></section>
      <section id="productos" class="codimas-section" style="background:#f4f4f1"><div class="codimas-shell"><div class="codimas-section-head"><div><p class="codimas-eyebrow">Productos de referencia</p><h2 class="codimas-section-title">Selecciona y <strong>agrega a cotización.</strong></h2></div></div><div class="codimas-feature-categories">${(category.featuredProducts || []).map((product) => `<article class="codimas-product-card"><div class="codimas-product-image"><img src="${product.image_url || heroImage}" alt="${product.name}" loading="lazy"></div><div class="codimas-product-meta"><div class="category">${product.brand || ''}</div><h3>${product.name}</h3><p>${product.spec || ''}</p><div class="code">Código ref. ${product.code || 'S/C'} · Unidad ${product.unit || 'consultar'}</div></div><button class="quote-add-btn codimas-btn codimas-btn-dark" data-name="${product.name}" data-code="${product.code || ''}" data-category="${category.title}" data-spec="${product.spec || ''}">Agregar a cotización</button></article>`).join('')}</div></div></section>
      <section class="codimas-section"><div class="codimas-shell"><div class="codimas-section-head"><div><p class="codimas-eyebrow">Aplicaciones</p><h2 class="codimas-section-title">Pensado para <strong>uso profesional.</strong></h2></div></div><div class="codimas-trust-strip">${(category.applications || []).map((item) => `<div class="codimas-trust-item"><strong>${item}</strong><span>Disponible para evaluar dentro de tu solicitud.</span></div>`).join('')}</div></div></section>
      ${related.length ? `<section class="codimas-section" style="background:#f4f4f1"><div class="codimas-shell"><div class="codimas-section-head"><div><p class="codimas-eyebrow">También puede interesarte</p><h2 class="codimas-section-title">Categorías <strong>relacionadas.</strong></h2></div><a class="codimas-link-arrow" href="../index.html#categorias">Ver todas →</a></div><div class="codimas-feature-categories">${related.map((item) => `<a class="codimas-feature-category" href="${item.slug}.html"><img src="${imageFor(item)}" alt="${item.title}" loading="lazy"><div class="codimas-feature-category-copy"><h3>${item.title}</h3><span>Explorar →</span></div></a>`).join('')}</div></div></section>` : ''}
      <section class="codimas-section-compact" style="background:#ffd200"><div class="codimas-shell" style="display:flex;justify-content:space-between;align-items:center;gap:24px;flex-wrap:wrap"><div><p class="codimas-eyebrow" style="color:#0b0b0d">Cotización</p><h2 class="codimas-section-title" style="margin-top:8px">¿Necesitas esta categoría?</h2></div><a class="codimas-btn codimas-btn-dark" href="../cotizacion.html?categoria=${encodeURIComponent(category.title)}">Enviar solicitud</a></div></section>
    </main>

    <footer class="codimas-footer"><div class="codimas-shell"><div class="codimas-footer-grid"><div><img src="${global.logo_negative_url || '../public/images/codimas-logo-negative.svg'}" alt="Codimas SpA" class="codimas-footer-logo"><p>${global.tagline || 'Productos, suministros y soluciones.'}</p></div><div><h3>Productos</h3><a href="../index.html#categorias">Categorías</a><a href="../cotizacion.html">Solicitar cotización</a></div><div><h3>Empresas</h3><a href="../index.html#empresas">Abastecimiento</a><a href="../index.html#servicios">Servicios</a></div><div><h3>Ayuda</h3><a href="../seguimiento.html">Seguimiento</a><a href="../index.html#contacto">Contacto</a></div><div><h3>Contacto</h3><a href="mailto:${global.sales_email || 'ventas@codimas.cl'}">${global.sales_email || 'ventas@codimas.cl'}</a><a href="tel:${(global.phone || '+56933365549').replace(/[^+\d]/g,'')}">${global.phone || '+56 9 3336 5549'}</a><a href="../admin/login.html">Administración</a></div></div><div class="codimas-footer-bottom"><span>© 2026 Codimas SpA.</span><span>${global.footer_country || 'Chile · Productos, suministros y soluciones.'}</span><span class="codimas-focusone-credit">${global.developer_credit || 'Sitio diseñado por Focus One SpA'}</span></div></div></footer>
  `;
