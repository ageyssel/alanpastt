...
function showAlert(message, type = 'error') {
  ...
  alertBox.className = `mb-8 p-4 font-bold ${
...
function renderTracking(data) {
  ...
  document.getElementById('result-responses').innerHTML = responses.length
    ? responses.map(item => `
        <article class="bg-white p-5 border border-slate-200">
      ...
      `).join('')
    : '<p class="bg-white border border-slate-200 p-4 text-slate-500 font-bold">Aún no hay respuestas registradas.</p>';
  ...
  document.getElementById('result-attachments').innerHTML = attachments.length
    ? attachments.map(file => `
      ...
      `).join('')
    : '<p class="bg-white border border-slate-200 p-4 text-slate-500 font-bold">Sin archivos visibles para esta solicitud.</p>';
