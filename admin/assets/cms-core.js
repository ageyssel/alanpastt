...
function setPath(path,val){const parts=path.split('.');let c=window.CODIMAS_ADMIN.site;parts.forEach((p,i)=>{if(i===parts.length-1)c[p]=val;else{if(c[p]===undefined)c[p]={};c=c[p]}});const mirrors={'global.sales_email':'#contact-sales-email','global.contact_email':'#contact-email','global.whatsapp':'#contact-whatsapp','global.whatsapp_message':'#contact-whatsapp-message','global.tagline':'#contact-footer'};const mirror=mirrors[path]?$(mirrors[path]):null;if(mirror&&mirror.value!==String(val??''))mirror.value=String(val??'');if(path==='global.whatsapp'&&window.CODIMAS_ADMIN.site.global)window.CODIMAS_ADMIN.site.global.phone=val?`+${String(val).replace(/\D/g,'')}`:window.CODIMAS_ADMIN.site.global.phone;syncRaw()}
...
async function saveAll(){
  ...
  try{
    window.dispatchEvent(new CustomEvent('codimas:before-save'));
    collectContact();
