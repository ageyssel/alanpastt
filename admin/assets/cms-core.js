...
function bindUploads(root=document){$$('.cms-upload-btn',root).forEach(btn=>{if(btn.dataset.bound)return;btn.dataset.bound='1';btn.addEventListener('click',()=>{const id=btn.dataset.targetId||btn.dataset.target;$(`.cms-file-input[data-target-id="${id}"],.cms-file-input[data-target="${id}"]`)?.click()})});$$('.cms-file-input',root).forEach(inp=>{if(inp.dataset.bound)return;inp.dataset.bound='1';inp.addEventListener('change',async()=>{const file=inp.files?.[0];if(!file)return;const id=inp.dataset.targetId||inp.dataset.target;const target=document.getElementById(id);const previousValue=target?.value||'';try{status('Subiendo imagen...');const url=await upload(file);if(target){target.value=url;target.dispatchEvent(new Event('input',{bubbles:true}));let p=target.closest('.cms-field')?.querySelector('.cms-media-preview');if(!p){p=document.createElement('img');p.className='cms-media-preview';target.closest('.cms-field')?.appendChild(p)}if(p)p.src=url}const publishNow=target&&(target.dataset.cmsPath||target.id==='hero-image-url');if(publishNow){status('Imagen subida. Publicando y verificando...');const saved=await saveAll({flushDrafts:false});if(!saved)throw new Error('No se pudo confirmar la publicación de la imagen.');status('Imagen publicada y verificada en el sitio público.','success')}else status('Imagen subida correctamente. Guarda el elemento para publicarla.','success')}catch(e){console.error('[Codimas CMS] Error al cambiar imagen:',e);if(target){target.value=previousValue;target.dispatchEvent(new Event('input',{bubbles:true}))}status(`No se pudo cambiar la imagen: ${e.message}`,'error')}finally{inp.value=''}})})}
